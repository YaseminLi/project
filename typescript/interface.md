...
//ming:直接传入对象字面量时会检查每个属性是否和类型匹配
printLabel({name:12,label:"hello label"})//error
```
...
let myObj = {size: 10, label: 'Size 10 Object'}
printLabel(myObj)
//ming:直接传入对象字面量时会检查每个属性是否和类型匹配,就是后面会讲到的额外的属性检查
printLabel({name:12,label:"hello label"})//error
```
...
ro.push(5) // error!
ro.length = 100 // error!
a = ro // error! ming:a的类型为number[]是可变的，ro是不可变的
```
...
interface SquareConfig {
  color?: string
  width?: number
  [propName: string]: any //ming:可以用于多个属性
}
```
...
//ming:但是下面这种就不会报错，因为dog是animal的子类型
interface Okay{
    [x:number]:Dog
    [x:string]:Animal
}
let cat:Okay[0]={name:'a',breed:'b'} //ok
```
...
接口描述了类的公共部分，而不是公共和私有两部分(ming:私有部分，前面有static)。 它不会帮你检查类是否具有某些私有成员。

### 类静态部分与实例部分的区别

当你操作类和接口的时候，你要知道类是具有两个类型的：静态部分的类型和实例的类型。 你会注意到，当你用构造器签名（ming:构造器属于静态类型）去定义一个接口并试图定义一个类去实现这个接口时会得到一个错误：
...
看下面的例子，我们定义了两个接口，  `ClockConstructor` 为构造函数所用和 `ClockInterface` 为实例方法所用。 为了方便我们定义一个构造函数 `createClock`，它用传入的类型创建实例。(ming:这个例子看不懂啊……)？
...
ming:其实就是，私有成员private state  只能通过extends方法遗传给子类，如selectableControl\button\textbox,而子类不能通过实现的方法去继承私有成员，就像image没有state一样

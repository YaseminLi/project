...
当成员被标记成 `private` 时，它就不能在声明它的类的外部被访问。比如：
...
//ming:
dog.say()//my name is dog   调用类的方法，方法内部调用的name属性属于在类的内部调用
dog.move() //error,move方法只能在类的内部调用

class Person {
  private constructor(name: string) { //私有构造器
    this.name = name
  }
}
let child=new Person("a") //error 构造器为私有属性
class Woman extends Person{} //error 构造器为私有属性
```
...
john.name = 'peter' //error name属性只能读，不能修改
```
...
let employee = new Employee()
employee.fullName = 'Bob Smith' //set方法
if (employee.fullName) {//get方法
  console.log(employee.fullName)
}
```
...
到目前为止，我们只讨论了类的实例成员，那些只有在类被实例化的时候才会被初始化的属性。 我们也可以创建类的静态成员，这些属性存在于类本身上面而不是类的实例上。 在这个例子里，我们使用 `static` 定义 `origin`，因为它是所有网格都会用到的属性。 每个实例想要访问这个属性的时候，都要在 `origin` 前面加上类名。 如同在实例属性上使用 `this.xxx` 来访问属性一样，这里我们使用 `Grid.xxx` 来访问静态属性。
...
抽象类中的抽象方法不包含具体实现并且必须在派生类中实现。 抽象方法的语法与接口方法相似。两者都是定义方法签名但不包含方法体。 然而，抽象方法必须包含 `abstract` 关键字**并且可以包含访问修饰符**。
...
department.generateReports() // 错误: 方法在声明的抽象类中不存在，department是Department的类型
```
